...
def remove_apidoc_artifacts(app):
    if not hasattr(remove_apidoc_artifacts, "done"):
        ...
        os.remove("manual/autogenerated/nexxT.rst")
...
def setup(app):
    ...
    shutil.rmtree("../c++/xml", ignore_errors=True)
    os.makedirs("manual/autogenerated", exist_ok=True)
    with open("manual/autogenerated/nexxT-gui.txt", "wb") as f:
        f.write(subprocess.check_output([os.path.split(sys.executable)[0] + "/nexxT-gui", "--help"]))
    with open("manual/autogenerated/nexxT-console.txt", "wb") as f:
        f.write(subprocess.check_output([os.path.split(sys.executable)[0] + "/nexxT-console", "--help"]))
    with open("manual/autogenerated/introduction.md", "w") as f:
        f.writelines(
            ["# Introduction\n", "\n"] +
            list(open("../README.md", "r").readlines())[2:]
        )
